  ...
  return (
    <div style={{ padding: 16, height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ fontSize: 10, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600, color: text.faint, letterSpacing: "0.1em" }}>SCHEMAS</div>
        <button onClick={() => setShowCreate(!showCreate)}
          style={{ padding: "3px 8px", borderRadius: 4, border: `1px solid ${showCreate ? palette.emerald + "44" : border.default}`, background: showCreate ? `${palette.emerald}1a` : "transparent", color: showCreate ? palette.emerald : text.faint, fontSize: 10, fontFamily: "'IBM Plex Mono', monospace", cursor: "pointer" }}>
          + New
        </button>
      </div>

      {showCreate && (
        <div style={{ marginBottom: 12, padding: 10, borderRadius: 6, border: `1px solid ${border.default}`, background: surface.card }}>
          ...
          <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
            <button onClick={handleCancelCreate}
              style={{ padding: "4px 10px", borderRadius: 4, border: `1px solid ${border.default}`, background: "transparent", color: text.faint, fontSize: 10, fontFamily: "'IBM Plex Mono', monospace", cursor: "pointer" }}>
              Cancel
            </button>
            <button onClick={handleCreate}
              style={{ padding: "4px 10px", borderRadius: 4, border: `1px solid ${palette.emerald}44`, background: `${palette.emerald}1a`, color: palette.emerald, fontSize: 10, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600, cursor: "pointer" }}>
              Create
            </button>
          </div>
        </div>
        ...
        {schemas.map(([id, schema]) => {
          ...
          return (
            <button key={id} onClick={() => onSelect(id)}
              style={{ display: "block", width: "100%", textAlign: "left", padding: "7px 10px", marginBottom: 2, borderRadius: 6, border: isSelected ? `1px solid ${palette.cyan}44` : "1px solid transparent", background: isSelected ? `${palette.cyan}1a` : "transparent", color: isSelected ? palette.cyan : text.secondary, fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", cursor: "pointer", transition: "all 0.15s" }}
              onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = surface.cardHover; }}
              onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = "transparent"; }}>
              <div>{schema.name || id}</div>
              <div style={{ fontSize: 9, color: text.hint, marginTop: 2 }}>{schema.fields.length} field{schema.fields.length !== 1 ? "s" : ""}</div>
            </button>
          );
        })}
      </div>
...
export function SchemaWorkbench() {
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 160px)" }}>
      <div style={{ width: 260, flexShrink: 0, borderRight: `1px solid ${border.default}`, overflowY: "auto" }}>
        <SchemaList schemas={typedSchemas} selectedId={selectedId} onSelect={setSelectedId} onCreate={handleCreate} />
      </div>

      <div style={{ flex: 1, minWidth: 0 }}>
        {!selectedSchema && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: text.hint, fontSize: 13, fontStyle: "italic" }}>
