...
export interface PathFinderProps {
  ...
  nodeColor?: (uri: string) => string;
  edgeColor?: (label: string) => string;
  defaultExcludedEdges?: Set<string>;
  title?: string;
  description?: string;
  maxDepth?: number;
  maxPaths?: number;
...
function SubwayMap({
  ...
  nodeKind: (uri: string) => string;
  nodeColorFn: (uri: string) => string;
  onSelectNode: (uri: string) => void;
    ...
    >
      ...
      >
        ...
        {[...dagNodes.entries()].map(([uri, node]) => {
          ...
          const color = nodeColorFn(uri);
          const label = nodeLabel(uri);
          const kind = nodeKind(uri);
...
export function PathFinder({
  ...
  nodeColor,
  edgeColor,
  defaultExcludedEdges,
  title = "Pathway Finder",
  description = "Find connection paths between any two entities.",
  maxDepth = 5,
  maxPaths = 8,
  ...
  const defaultNodeColor = useCallback((_uri: string) => theme.text.muted, [theme.text.muted]);
  const defaultEdgeColor = useCallback((_label: string) => theme.text.faint, [theme.text.faint]);

  const nodeColorFn = nodeColor ?? defaultNodeColor;
  const edgeColorFn = edgeColor ?? defaultEdgeColor;
  ...
  const [disabledEdges, setDisabledEdges] = useState<Set<string>>(() => new Set(defaultExcludedEdges ?? []));
  ...
  const handleFind = useCallback(() => {
    ...
    setSearching(true);
    setTimeout(() => {
      const found = findPaths(filteredAdj, startUri, endUri, maxDepth, maxPaths);
      setPaths(found);
      setSearching(false);
    }, 10);
  }, [startUri, endUri, filteredAdj, maxDepth, maxPaths]);

  const nodeLabelFn = useCallback((uri: string): string => {
    const n = nodes.get(uri);
    if (!n) return uri.split("/").pop() || uri;
    return n.label;
  }, [nodes]);

  const nodeKindFn = useCallback((uri: string): string => {
    return nodes.get(uri)?.kind || "";
  }, [nodes]);
            ...
            {filtersOpen && (
              <div style={{
                ...
                <div style={{
                  ...
                  >Enable all</span>
                  {defaultExcludedEdges && defaultExcludedEdges.size > 0 && (
                    <>
                      <span style={{ color: theme.border.medium }}>|</span>
                      <span
                        onClick={() => setDisabledEdges(new Set(defaultExcludedEdges))}
          ...
          {paths.length === 0 ? (
            <div style={{
              ...
              <div style={{ color: theme.text.faint, fontSize: sz(13) }}>
                {emptyMessage} within {maxDepth} steps
              </div>
            ...
            <div>
              <div style={{
                ...
                {paths.length >= maxPaths && " (showing first " + maxPaths + ")"}
              </div>
