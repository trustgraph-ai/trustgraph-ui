...
function defaultFormatLabel(v: number): string {
  return v < 0 ? `${Math.abs(v)} BC` : String(v);
}
...
export function EventTimeline({
  ...
  const maxYear = useMemo(() => buckets.length > 0 ? buckets[buckets.length - 1].year + binSize : 2025, [buckets, binSize]);
